...
static void	SD_send_command(byte command, byte arg1, byte arg2, byte arg3, byte arg4, byte CRC);
static bool	SD_read_sector(unsigned int sector);



bool			SD_is_SDHC = false;						// If the SD card is SDHC (High Capacity SD)
bool			SD_is_initialized = false;					// If the SD card has been initialized via SD_init();
unsigned long	SD_current_sector = 0;					// The address of the sector on buffer
...
bool	SD_init(void) {
	...
	int count = 10;
	while (count--)	{ SPI_queue(0b11111111); }
	SPI_send();

	// "After the 74 cycles (or more) have occurred, your program should set the CS line to 0"
	...
	} while (!(SPI_get_response(1, GRACE_BYTES+1, 0xff, 1, &r1_response) && r1_response == 0x1));
	/*==============================[/CMD0]===============================*/
	...
	//                              |     |            \_ CRC
	//                              |      \_  Check Pattern (It's arbitrary but should be echoed back)
	...
	do {
		...
		if	(!SPI_get_response(1, GRACE_BYTES+1, 0xff, 1, &r1_response) || r1_response != 0x1) {
			/* NON USABLE CARD */
			return (false);
		} else {
			// CMD 55 OK!
		...
		/*===========================[/ACMD41]============================*/
	} while(!SPI_get_response(1, GRACE_BYTES+1, 0xff, 1, &r1_response) || r1_response != 0x0);

		
	
	/*===============================[CMD58]==============================*/
	SD_send_command(CMD58, 0x0, 0x0, 0x0, 0x0, 0xff);
	if	(!SPI_get_response(6, GRACE_BYTES+1, 0xff, 1, r7_response) || (r7_response[1] & 0b10000000) != 0b10000000) {
		/* CARD INIT FAILED */
		return (false);
	} else {
		if ((r7_response[1] & 0b01000000) != 0b01000000) {
			// Version 2.00 or later, SDHC or SHXC, SD Card
			SD_is_SDHC = true;
		} else {
			// Version 2.00 or later, Standard Capacity, SD Card
			SD_is_SDHC = false;
		}
	}
	/*==============================[/CMD55]==============================*/
	...
	/*===============================[CMD59]==============================*/
		...
		if	(!SPI_get_response(1, GRACE_BYTES+1, 0xff, 1, &r1_response) || r1_response != 0x0) {
			/* NON USABLE CARD */
			return (false);
		} else {
			// CMD 59 OK!
...
// SD_read_byte: Reads a byte at the specified sector and relative address and writes it to b
bool	SD_read_byte(unsigned int sector, unsigned int relative_address, byte *b) {
	byte retries = 5;
	while (sector != SD_current_sector || !SD_good_sector) { // If the sector is not the one on the buffer or while the sector is just bad read the sector
		if (retries == 0) { return (false); } // Put a limit on the number of tries
		SD_read_sector(sector);
		retries--;
	}
	if (relative_address < 512) // If the address is outside of the sector, return it
...
bool	SD_read_bytes(unsigned int sector, unsigned int start_rel_address, size_t bytes, void *b)
{
	size_t size = bytes;
	byte tmp;
	while (bytes > 0) {
...
// [INTERNAL] SD_send_command: Sends a command to the SD Card. Note you have to get the response by hand with SPI_get_response
static void	SD_send_command(byte command, byte arg1, byte arg2, byte arg3, byte arg4, byte CRC)
{
...
{	
	...
	if (SD_sector_buffer[0] != 0x0)	// If the first response byte is not 0x0 it means error so mark the sector as bad
		SD_good_sector = false;
	...
	while (++i < GRACE_BYTES)
	{
		...
		{
			SD_sector_pad = i + 1;			// The next byte is the start of the sector data
		}
	}
	if (SD_sector_pad == GRACE_BYTES + 1)	// If no 0xfe byte has been detected, then mark the sector as bad
		SD_good_sector = false;
...
bool SD_write_sector(unsigned int sector, char *buffer)
{
	...
	int len = 512;
	while (len--) {
		if (!SPI_queue(*(buffer++))) { return (false); }
	}
	SPI_queue(0x00); // CRC
	SPI_queue(0x00); // CRC
	SPI_send();
	if	(!SPI_get_response(1, (GRACE_BYTES + 1), 0xff, 1, &r1_response))
		return (false);
	if	(!SPI_get_response(1, (GRACE_BYTES + 1), 0xff, 0, &r1_response) || r1_response != 0xff)
		return (false);
