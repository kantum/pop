...
void	SPI_init(void) {
		...
		SPI1BRG = 0x1; // BRG = Baud Rate Generator, SPI clock frequency for proper communication with the SD
...
void	SPI_slave_select(byte slave) {
	if (slave == SPI_SD) {
		SPI_SS_LAT_SD = 0;
	} else {				// This is for slave being SPI_NONE or as a fallback for other values
		SPI_SS_LAT_SD = 1;
	}
}
...
bool	SPI_get_byte(byte *b) {
	if (!SPI_available()) { return (false); }	// If there are no more characters to read return false
...
bool	SPI_queue_byte(byte b, bool discard) {
	if (SPI_TX_size < SPI_BUFF_SIZE) {			// Check there is enough space in the buffer
...
bool	SPI_read(size_t bytes, byte dummy) {
	while (bytes--)
		if (!SPI_queue_byte(dummy, false)) { return (false); }
	SPI_send();
	return (true);
}


//	SPI_get_response: This function helps with the SPI response processing as it aligns automatically the bytes and waits for a response to be sent as this function has a lot of arguments let me explain them:
/*
 * response_size = The response size you are expecting (in bytes)
 * wait_bytes = The total number of bytes that have to be read (must be higher than response_size and it's to allow the slave a delay in the response). A good value could range from 8-100; 
 * dummy = the "dummy" byte to write during those wait_bytes (to keep the clock ticking)
 * sleeping_bit = Which bit 0 or 1 is supposed to be the MISO line during no-response time
 * *response = the char or char array where the aligned response will be stored 
 */
//bool	SPI_get_response(size_t response_size, size_t wait_bytes, byte dummy, bool sleeping_bit, byte *response) {
//
//	if (SPI_available()) { return (false); }				// Buffer not empty
//
...
//		if (algnmt == 9) {								// We haven't yet started to recompose bytes
//			if (current != sleeping_byte) {
//				algnmt = SPI_misaligned_bits(current, sleeping_bit);
//			}
//		} else {										// We have started recomposing bytes
