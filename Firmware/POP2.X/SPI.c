...
void	SPI_init(void)
{
	...
	IPC7bits.SPI1IP	  = 6;			/* Interrupt Priority */
	IFS1bits.SPI1RXIF = 0;			/* Reset the interrupt flag */
	IEC1bits.SPI1RXIE = 1;			/* Enable the interrupt */
	/* [/SPI Receive Done Interrupt Setup] */
...
void	SPI_send(void)
{
	...
	IFS1bits.SPI1RXIF = 1;		// If it's the first byte activate the "domino" effect by setting the flag and in consequence call the interrupt function;
...
void    __ISR (_SPI_1_VECTOR, IPL7SOFT) SPI(void)
{
	IFS1bits.SPI1RXIF = 0;									// Reset Interrupt Flag
