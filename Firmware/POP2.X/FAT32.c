...
//SD_recover
bool FAT32_recover(void)
{
	if (FAT32_working) return (true);
	int rt = 1;
	while (rt--) {
		if (FAT32_mount()) { return (FAT32_working = true); }
		//delay_ms(1);
	}
	return (false);
}

bool FAT32_crashed(void) {
	SD_working = false;
	FAT32_working = false;
	SD_recover();
	FAT32_recover();
...
{
	if (!FAT32_working && !FAT32_recover()) return (false);
...
bool	FAT32_fputlb(file_t *file, char *b)
{
	...
	if (!SD_write_sector(file->cursor_sector, b)) { return (FAT32_crashed()); }
...
{
	if (!FAT32_working) FAT32_recover();
