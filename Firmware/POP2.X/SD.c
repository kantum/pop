...
//SD_recover
bool SD_recover(void)
{
	if (SD_working) return (true);
	int rt = 1;
	while (rt--) {
		if (SD_init()) { return (SD_working = true); }
		//delay_ms(1);
	}
	return (false);
}

bool SD_crashed(void) {
	SD_working = false;
	FAT32_working = false;
	SD_recover();
	FAT32_recover();
...
{
	if (!SD_working && !SD_recover()) return (false);
	...
	{ // If the sector is not the one on the buffer or while the sector is just
	  // bad read the sector
		if (retries == 0)
			return (SD_crashed()); // Put a limit on the number of tries
